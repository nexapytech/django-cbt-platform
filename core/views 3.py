...
def initialize_subscription(request):
    if request.method == 'POST':
        try:
            # Retrieve CSRF token and user details
            csrf_token = get_token(request)
            user = request.user
            user_email = user.email

            # Example: Fetch the plan amount based on the subscription selected by the user.
            # For this example, we will assume 'basic' plan has ₦10,000 amount (converted to kobo)
            amount = 3000000  # ₦10,000 = 1000000 Kobo

            # Prepare the payload for Paystack API
            payload = {
                "email": user_email,
                'username': user.username,
                "callback_url": request.build_absolute_uri('/dashboard'),  # Fixed typo
                "amount": amount  # Amount in kobo (100 kobo = 1 Naira)
            }

            # Prepare the headers for Paystack API request
            headers = {
            ...
            # Debug: Print the response status and data for troubleshooting
            ...
            if response.status_code == 200:
               ...
               #print("Response JSON Data:", response_data)
                ...
                # Check if 'data' exists and contains 'authorization_url'
                if 'data' in response_data and 'authorization_url' in response_data['data']:
                    payment_url = response_data['data']['authorization_url']
                    return redirect(payment_url)
                else:
                    return JsonResponse({'status': 'error', 'message': 'Invalid response from Paystack'})

            else:
                # Handle other errors from Paystack API
                return JsonResponse({'status': 'error', 'message': 'Failed to initiate payment'})

        except requests.exceptions.RequestException as e:
            # Handle network-related errors
            #print("Error occurred while sending request to Paystack:", str(e))
            return JsonResponse({'status': 'error', 'message': 'Error occurred while communicating with Paystack'})
...
def verify_subscription(request, profile):
    if request.method == 'GET':
        csrf_token = get_token(request)
        # Extract the reference from the query parameters
        reference = request.GET.get('reference', '')

        if reference:
            # Use the reference to verify the payment with Paystack
            ...
            try:
                ...
                if response.status_code == 200 and response_data['data']['status'] == 'success':
                    ...
                    user = request.user
                    # Assuming you have a UserProfile model with subscription fields
                    user_profile =  profile
                    user_profile.subscription_active = True
                    user_profile.subscription_expires_at = now + timedelta(days=30)  # Set expiry to 30 days from now
                    user_profile.save()

                    # Render success page
                    return redirect('/dashboard')
                else:
                    # Payment failed, render failure page
                     messages.info(request, 'payment failed contact support')
                     return  redirect('subscription')

            except requests.exceptions.RequestException as e:
                # Handle any network issues or Paystack API errors
...
def mark_exam(candidate_id, exam_id):
    ...
    for question in exam_questions:
        question_id = str(questioiun.get("id"))
        ...
        if request.method == "GET":
            ...
            if verification.make_token == token:
                ...
                if verification.is_email_verified:
                    ...
                    verification.save()
                    return HttpResponse('Email verified, kindly login')
            else:
...
def verification_success(request):
    if request.get_signed_cookie('verification_success'):
...
def dashboard(request):
    ...
    now = timezone.now()
    profile = get_object_or_404(UserProfile , user = request.user)
    verify_subscription(request, profile)
    if profile.subscription_expires_at is not None and profile.subscription_expires_at < now:
...
def verify_exam_login(request):
        if request.method == 'POST':
            try:
                ...
                candidates = [candidate for candidate in candidate_list(file=exam_settings.uploaded_file, exam=exam)]
...
def end_exam(request):
    ...
    try:
        ...
        if exam_settings.enable_feedback and profile.subscription_active:
