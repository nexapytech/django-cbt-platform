...
def Send_Email_Verification_msg(request, user):
    try:
        ...
        send_mails = EmailMultiAlternatives(
...
def mark_theory_answer(
    candidate_answer,
    correct_answer,
    threshold=0.7,
    key_point_weight=0.4,
    semantic_weight=0.5,
    ...
    matched_points = len(
        set(candidate_key_points).intersection(set(correct_key_points))
    )
    completeness_score = (
        matched_points / len(correct_key_points) if correct_key_points else 0
    )

    # Assess relevance
    irrelevant_points = len(set(candidate_key_points) - set(correct_key_points))
    relevance_penalty = (
        irrelevant_points / len(candidate_key_points) if candidate_key_points else 0
    )
...
def mark_exam(candidate_id, exam_id):
    ...
    candidate_answer_record = get_object_or_404(
        ExamAnswer, candidate_id=candidate_id, exam=exam
    )
    ...
    for question in exam_questions:
        ...
        candidate_response = candidate_answers_dict.get(question_id, {}).get(
            "answers", ""
        )
            ...
            if User.objects.filter(email=email).exists():
                ...
                try:
                    ...
                    executor = ThreadPoolExecutor(
                        max_workers=1
                    )  # Optionally, set the max_workers to 1 since it's just one task.
            ...
            if verification.make_token == token:
                ...
                if verification.is_email_verified:
                    ...
                    response.set_signed_cookie(
                        "verification_success", "true", salt="email", max_age=400
                    )
                    return response
            else:
...
def verification_success(request):
    ...
    return render(
        request, "activate_email.html", {"message": "Invalid or expired verification."}
    )
...
def dashboard(request):
    ...
    context = {
        ...
        "email_verified": (
            email_verify.is_email_verified if email_verify else False
        ),  # Use a fallback if email_verify is None
        "profile": profile,
    }
        ...
        if request.method == "POST" and request.FILES["questions_excel_file"]:
            ...
            try:
                ...
                if not all(col in df.columns for col in required_columns):
                    return JsonResponse(
                        {
                            "error": "The Excel file does not contain the required columns."
                        }
                    )
                ...
                for index, row in df.iterrows():
                    question = {
                        ...
                        "title": row["Title"],
                        "answers": (
                            row["Answers"] if not pd.isna(row["Answers"]) else ""
                        ),
                        "options": (
                            [opt.strip() for opt in str(row["Options"]).split(",")]
    ...
    try:
        ...
        if request.method == "POST":
            if exam_id:
                ...
                exam.delete()
                return JsonResponse(
                    {"message": "Exam deleted successfully"}, status=200
                )
...
def delete_response(request, exam_id, candidate_id):
    if request.method == "POST":
        if candidate_id:
            exam = get_object_or_404(Exam, uuid=exam_id)
            candidate_response = get_object_or_404(
                ExamAnswer, exam=exam, candidate_id=candidate_id
            )
...
def parse_csv(file):
    ...
    with file.open("r") as f:
        ...
        reader = csv.reader(f)
        candidates = [
            row[0].strip() for row in reader if row
        ]  # Extract names or values from the CSV
    return candidates
...
def parse_excel(file):
    ...
    candidates = (
        df.iloc[:, 0].dropna().astype(str).tolist()
...
def verify_exam_login(request):
    if request.method == "POST":
        if request.session.get("is_logging_in"):
            return JsonResponse(
                {"backend": "Login already in progress. Please wait."}, status=429
            )
        ...
        try:
            ...
            candidates = [
                ...
                for candidate in candidate_list(
                    file=exam_settings.uploaded_file, exam=exam
                )
            ]
            ...
            if candidate_id in candidates:
                ...
                if ExamAnswer.objects.filter(
                    candidate_id=candidate_id, exam=exam
                ).exists():
                    request.session.pop("candidate_id", None)

                    return JsonResponse(
                        {
                            "candidate_exists": "You have already participated in the exam"
                        }
                    )
...
def view_exam(request, uuid, published_hash):
    ...
    exam_method_context = {"id": exam_settings, "exam": exam}
    ...
    try:
        ...
        try:
            is_candidate_login = get_object_or_404(
                ExamAnswer, exam=exam, candidate_id=candidate_id
            )

            if (
                is_candidate_login.is_candidate_logined
                and not is_candidate_login.end_exam
            ):
                ...
                if (
                    ...
                    old_session = int(
                        request.session.get("reload_count", 0)
                    )  # Default to 0 if not set
                    new_session = old_session + 1  # Increment the old session count
                    request.session["reload_count"] = (
                        new_session  # Update the session with the new count
                    )
                    # Debug print to check values
                    print(
                        f"This is old session: {old_session}, new session: {new_session}"
                    )
...
def my_exam_settings(request):
    ...
    if request.method == "POST":
        ...
        candidate_method = request.POST.get(
            "candidate-method"
...
def save_exam_answer(request):
    if request.method == "POST":
        try:
            ...
            examAnswer = get_object_or_404(
                ExamAnswer, exam=exam, candidate_id=candidate_id
            )
            if (
                exam_uuid == str(exam.uuid)
                and publish_hash == exam.published_hash
                and candidate_id
            ):
...
def end_exam(request):
    if request.method != "POST":
        if request.session.get("is_end_exam"):
            return JsonResponse(
                {"backend": "exam ended clicked. Please wait."}, status=429
            )
    ...
    try:
        ...
        if not candidate_id:
            return JsonResponse(
                {"success": False, "message": "Candidate not logged in."}
            )
        ...
        exam_candidate = get_object_or_404(
            ExamAnswer, exam=exam, candidate_id=candidate_id
        )
        ...
        return JsonResponse(
            {"success": False, "message": "An error occurred: " + str(e)}
        )
    finally:
        # Always release the lock, even if error occurs
        request.session["is_logging_in"] = False
...
def resent_verification_email(user, current_site, uidb64, token, email_subject):
    email_body = render_to_string(
        "verification_center.html",
        {"user": user, "domain": current_site, "uidb64": uidb64, "token": token},
    )
    plain_text = strip_tags(email_body)
...
def resend_verification_link(request):
    try:
        ...
        return JsonResponse(
            {
                "success": "New email verification sent successfully. Kindly check your mails"
            }
        )
    except Exception as e:
        # Log the exception for debugging
        print(f"Error: {e}")
        return JsonResponse(
            {"error": "Something went wrong. Unable to send verification link"}
        )
